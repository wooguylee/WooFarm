...
(async () => {
  ...
  // 경작된 타일에 씨앗 심기
  await tiles[0].click();
  console.log('  ✅ 타일 0에 씨앗 심기');
  await page.waitForTimeout(300);
  
  await tiles[1].click();
  console.log('  ✅ 타일 1에 씨앗 심기');
  await page.waitForTimeout(300);
  ...
  await tiles[0].click();
  console.log('  ✅ 타일 0 물 주기');
  await page.waitForTimeout(200);
  
  await tiles[1].click();
  console.log('  ✅ 타일 1 물 주기');
  await page.waitForTimeout(200);
  ...
  tileStates.forEach(t => {
    console.log(`  타일 ${t.index}: state=${t.state}, crop=${t.crop}, growth=${t.growth}`);
  });
  
  // 시간 경과
  console.log('\n6️⃣ 시간 경과 (자동 성장)\n');
  
  // 다음 시간으로 진행 (TimeSystem.tick 호출)
  for (let i = 0; i < 10; i++) {
    await page.evaluate(() => {
  ...
  growthStates.forEach(t => {
    console.log(`  타일 ${t.index}: state=${t.state}, crop=${t.crop}, growth=${t.growth}, progress=${t.progress}`);
  });
  
  // 수확
  console.log('\n8️⃣ 수확\n');
  
  // 성숙도를 100으로 강제 설정 (테스트용)
  await page.evaluate(() => {
    window.FarmSystem.grid[0].growthProgress = 100;
    window.FarmSystem.grid[1].growthProgress = 100;
  });
  
  // 수확 도구 선택
  await toolSlots[3].click();
  console.log('  ✅ 수확 도구 선택');
  await page.waitForTimeout(200);
  
  // 타일 클릭해서 수확
  await tiles[0].click();
  console.log('  ✅ 타일 0 수확 시도');
  await page.waitForTimeout(300);
